...
- DR and DME reliability/calibration diagrams (confidence vs accuracy)
...
def _predict_from_expected_score(y_proba, boundaries, num_classes):
    expected_score = np.sum(y_proba * np.arange(num_classes, dtype=np.float32)[None, :], axis=1)
    pred = np.searchsorted(boundaries, expected_score, side="right")
    return np.clip(pred.astype(np.int32), 0, num_classes - 1)


def _optimize_expected_score_thresholds(y_true, y_proba, num_classes, grid_size=41, n_passes=3, min_gap=1e-3):
    if num_classes <= 1:
    ...
    def _obj(b):
        ...
        return float(compute_quadratic_weighted_kappa(y_true, p, num_classes)), p

    best_qwk, best_pred = _obj(boundaries)
    for _ in range(max(1, int(n_passes))):
        improved = False
        for idx in range(len(boundaries)):
            lo = 0.0 if idx == 0 else boundaries[idx - 1] + min_gap
            hi = (float(num_classes - 1) if idx == len(boundaries) - 1 else boundaries[idx + 1] - min_gap)
            if hi <= lo:
                continue
            for c in np.linspace(lo, hi, num=max(5, int(grid_size)), dtype=np.float32):
                trial = boundaries.copy(); trial[idx] = float(c)
                q, p = _obj(trial)
                if q > best_qwk:
                    best_qwk = q; boundaries[idx] = float(c); best_pred = p; improved = True
        if not improved:
            break
...
# ---------------------------------------------------------------------------
# Plot helpers (shared for DME and DR)
# ---------------------------------------------------------------------------

def plot_ordinal_error_histogram(y_true, y_pred, title="Ordinal Prediction Error Distribution", output_path="ordinal_error_hist.png"):
    ...
    ax.legend(handles=[Patch(facecolor="green", label="Correct (0)"),
                        Patch(facecolor="orange", label="Off-by-1"),
                        Patch(facecolor="red", label="Off-by-2+")], fontsize=9)
...
def plot_per_class_prediction_distribution(y_true, y_pred, class_names, title="Per-Class Prediction Distribution", output_path="per_class_pred_dist.png"):
    if not _PLOTTING_AVAILABLE:
        return
    n = len(class_names)
    ...
    bottoms = np.zeros(n)
    for j in range(n):
        ax.bar(class_names, matrix[:, j], bottom=bottoms, label=f"Predicted: {class_names[j]}",
               color=colors[j], edgecolor="black", linewidth=0.5, alpha=0.88)
        bottoms += matrix[:, j]
    ax.set_xlabel("True Class", fontsize=12); ax.set_ylabel("Proportion", fontsize=12)
    ax.set_title(title, fontsize=13); ax.set_ylim([0, 1.05])
...
def plot_multiclass_roc(y_true, y_proba, class_names, title="Multi-Class ROC Curves (One-vs-Rest)", output_path="roc_curves.png"):
    ...
    ax.set_title(title, fontsize=13); ax.legend(loc="lower right", fontsize=10); ax.grid(alpha=0.3)
    plt.tight_layout(); plt.savefig(output_path, dpi=150); plt.close(fig)
    logger.info("ROC curves saved to '%s'.", output_path)


def plot_joint_qwk_comparison(dme_qwk_raw, dr_qwk_raw, dr_qwk_calib, stage_label="Current Stage", output_path="joint_qwk_comparison.png"):
    if not _PLOTTING_AVAILABLE:
        return
    labels = ["DME QWK\n(raw)", "DR QWK\n(raw)", "DR QWK\n(calib)"]
    ...
    ax.set_ylim([0, 1.10]); ax.set_ylabel("QWK Score", fontsize=12)
...
def plot_reliability_diagram(y_true, y_proba, class_names, title="Calibration Reliability Diagram", output_path="reliability_diagram.png", n_bins=10):
    ...
    axes[0].plot(bin_confs, bin_accs, "o-", color="steelblue", linewidth=2, markersize=6, label="Model")
    axes[0].fill_between(bin_confs, bin_accs, bin_confs, alpha=0.2, color="tomato", label="Calibration gap")
    ...
    axes[0].legend(fontsize=9); axes[0].set_xlim([0, 1]); axes[0].set_ylim([0, 1]); axes[0].grid(alpha=0.3)
    axes[1].bar(bin_confs, bin_counts, width=(1.0 / n_bins) * 0.85, color="steelblue", edgecolor="black", alpha=0.8)
    axes[1].set_xlabel("Mean Predicted Confidence", fontsize=12)
    axes[1].set_ylabel("Sample Count", fontsize=12)
    axes[1].set_title("Confidence Distribution", fontsize=11); axes[1].grid(axis="y", alpha=0.3)
    plt.tight_layout(); plt.savefig(output_path, dpi=150); plt.close(fig)
    logger.info("Reliability diagram saved to '%s'.", output_path)


def plot_per_class_f1_bar(y_true, y_pred, class_names, title="Per-Class F1 Score", output_path="per_class_f1.png"):
    ...
    ax.set_title(title, fontsize=13); ax.legend(fontsize=9); ax.grid(axis="y", alpha=0.3)
    plt.tight_layout(); plt.savefig(output_path, dpi=150); plt.close(fig)
    logger.info("Per-class F1 bar chart saved to '%s'.", output_path)


# ---------------------------------------------------------------------------
# DR comprehensive dashboard
# ---------------------------------------------------------------------------

def plot_dr_dashboard(dr_metrics, output_dir, num_dr_classes=NUM_DR_CLASSES, dr_true=None, dr_pred=None, dr_proba=None):
    """2x3 DR grading evaluation dashboard: confusion matrices, F1, error hist, boundary confusion, summary."""
    if not _PLOTTING_AVAILABLE:
        return
    ...
    if cm_data is not None:
        ...
        cm = (sk_cm(dr_true, dr_pred, labels=list(range(num_dr_classes))) if _SKLEARN_AVAILABLE
              else np.array([[int(np.sum((dr_true == i) & (dr_pred == j))) for j in range(num_dr_classes)] for i in range(num_dr_classes)]))
    else:
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=names, yticklabels=names, ax=axes[0, 0])
    axes[0, 0].set_title("DR Confusion Matrix (Counts)", fontsize=12)
    axes[0, 0].set_xlabel("Predicted"); axes[0, 0].set_ylabel("True")

    sns.heatmap(cm_norm, annot=True, fmt=".2f", cmap="YlOrRd", xticklabels=names, yticklabels=names, vmin=0, vmax=1, ax=axes[0, 1])
    ...
    if dr_true is not None and dr_pred is not None and len(dr_true) > 0 and _SKLEARN_AVAILABLE:
        report = classification_report(dr_true, dr_pred, labels=list(range(num_dr_classes)),
        ...
        axes[0, 2].set_xlabel("Class"); axes[0, 2].set_ylabel("F1"); axes[0, 2].set_ylim([0, 1.10]); axes[0, 2].legend(fontsize=7)
    else:
    ...
    if boundary:
        ...
        axes[1, 1].set_xlabel("Class Boundary"); axes[1, 1].set_ylabel("Confusion Rate"); axes[1, 1].set_ylim([0, 1.05])
    ...
    axes[1, 2].axis("off")
    summary = (
        f"DR QWK (raw):   {qwk_raw:.4f}  {'OK' if qwk_raw  >= QWK_TARGET_THRESHOLD else 'X'}\n"
        f"DR QWK (calib): {qwk_cal:.4f}  {'OK' if qwk_cal  >= QWK_TARGET_THRESHOLD else 'X'}\n"
        f"Calibration applied: {'Yes' if calib.get('applied') else 'No'}\n"
        f"DR MAE:      {dr_mae:.3f}\n"
    ...
    axes[1, 2].text(0.05, 0.5, summary, transform=axes[1, 2].transAxes, fontsize=12,
...
def plot_comprehensive_dashboard(y_true, y_pred, y_proba, metrics, class_names=None, output_path="comprehensive_dashboard.png", num_classes=NUM_DME_CLASSES):
    """2x3 DME evaluation dashboard. Summary panel shows DME QWK (raw) AND DR QWK (calib)."""
    if not _PLOTTING_AVAILABLE:
        return
    names = class_names or DME_CLASS_NAMES[:num_classes]
    cm = (sk_cm(y_true, y_pred, labels=list(range(num_classes))) if _SKLEARN_AVAILABLE
          else np.array([[int(np.sum((y_true == i) & (y_pred == j))) for j in range(num_classes)] for i in range(num_classes)]))
    row_sums = cm.sum(axis=1, keepdims=True)
    cm_norm  = np.where(row_sums > 0, cm.astype(float) / row_sums, 0.0)
    ...
    axes[0, 2].set_xlabel("Class"); axes[0, 2].set_ylabel("F1"); axes[0, 2].set_ylim([0, 1.10])
    axes[0, 2].axhline(0.80, color="red", linestyle="--", linewidth=1, label="Target 0.80"); axes[0, 2].legend(fontsize=8)
    ...
    summary_text = (
        ...
        f"DR  QWK (calib): {dr_qwk_cal:.4f}  {'OK' if dr_qwk_cal  >= QWK_TARGET_THRESHOLD else 'X'}\n"
    ...
    axes[1, 2].text(0.05, 0.5, summary_text, transform=axes[1, 2].transAxes, fontsize=12,
                    verticalalignment="center",
...
def evaluate_dr_grading(dr_true, dr_pred, dr_proba, output_dir, num_dr_classes=NUM_DR_CLASSES,
                         calibrate_thresholds=False, min_qwk_gain=1e-4, max_accuracy_drop=0.0):
    if len(dr_true) == 0:
    ...
    calibration = {"enabled": bool(calibrate_thresholds), "applied": False,
                   "baseline_qwk": base_qwk, "baseline_accuracy": base_acc}
    ...
    if calibrate_thresholds and dr_proba is not None and len(dr_proba) > 0:
        ...
        calibration.update({"calibrated_qwk": float(calibrated_qwk), "calibrated_accuracy": calibrated_acc,
        ...
        if calibrated_qwk >= base_qwk + float(min_qwk_gain) and calibrated_acc >= base_acc - float(max_accuracy_drop):
            dr_pred_final = calibrated_pred.astype(np.int32)
            calibration["applied"] = True
            logger.info("DR threshold calibration applied: QWK %.4f -> %.4f", base_qwk, calibrated_qwk)
        else:
    ...
    if _SKLEARN_AVAILABLE:
        ...
        cm = np.array([[int(np.sum((dr_true == i) & (dr_pred_final == j))) for j in range(num_dr_classes)] for i in range(num_dr_classes)])

    dr_mae  = float(np.mean(np.abs(dr_true - dr_pred_final)))
    dr_rmse = float(np.sqrt(np.mean((dr_true - dr_pred_final) ** 2)))
    dr_ordinal = compute_ordinal_metrics(dr_true, dr_pred_final, num_dr_classes, class_names=DR_CLASS_NAMES[:num_dr_classes])

    logger.info("DR Grading Results: QWK(raw)=%.4f QWK(calib)=%.4f Acc=%.4f MAE=%.4f RMSE=%.4f",
                base_qwk, dr_qwk, dr_acc, dr_mae, dr_rmse)

    results = {"dr_qwk": dr_qwk, "dr_accuracy": dr_acc, "dr_mae": dr_mae, "dr_rmse": dr_rmse,
               "confusion_matrix": cm.tolist(), "calibration": calibration, "ordinal": dr_ordinal}
...
def evaluate_comprehensive(
    ...
    if calibrate_dme_thresholds:
        ...
        calibration["dme"].update({"baseline_qwk": baseline_qwk, "calibrated_qwk": float(calibrated_qwk),
                                    "boundaries": [float(x) for x in boundaries.tolist()],
    ...
    dr_metrics = evaluate_dr_grading(dr_true, dr_pred, dr_proba, output_dir=output_dir,
    ...
    interpretation = generate_medical_interpretation({
        "qwk": qwk_details["qwk"], "dr_qwk_calib": dr_qwk_calib,
        "mae": ordinal_metrics["mae"], "accuracy": accuracy,
        "f1_macro": f1_macro, "per_class": per_class})

    metrics = {
        "qwk": qwk_details["qwk"], "target_met": qwk_details["qwk"] >= QWK_TARGET_THRESHOLD,
        "mae": ordinal_metrics["mae"], "rmse": ordinal_metrics["rmse"],
        "accuracy": accuracy, "f1_macro": f1_macro, "f1_weighted": f1_weighted,
        "roc_auc": roc_auc_dme, "per_class": per_class, "ordinal": ordinal_metrics,
        "qwk_details": qwk_details, "dr": dr_metrics,
        "dr_qwk_raw": dr_qwk_raw, "dr_qwk_calib": dr_qwk_calib,
        "calibration": calibration, "interpretation": interpretation,
        "num_samples": int(len(dme_true)), "num_dr_samples": int(len(dr_true)),
        "class_counts": {dme_names[i]: int(np.sum(dme_true == i)) for i in range(num_dme_classes)},
    ...
    logger.info("DR  QWK (calib): %.4f %s", dr_qwk_calib,
                "(OK)" if dr_qwk_calib >= QWK_TARGET_THRESHOLD else "(below target)")
    logger.info("DME MAE:         %.3f  Accuracy: %.4f  F1(macro): %.4f",
    ...
    # ================================================================
    # VISUALISATIONS — DME and DR parity
    # ================================================================

    # DME: ordinal confusion matrix
    plot_ordinal_confusion_matrix(dme_true, dme_pred, class_names=dme_names,
        output_path=os.path.join(output_dir, "dme_ordinal_confusion_matrix.png"),
        num_classes=num_dme_classes, title=f"DME Ordinal Confusion Matrix - {stage_label}")

    # DME: comprehensive dashboard (2x3)
    plot_comprehensive_dashboard(dme_true, dme_pred, dme_proba, metrics=metrics,
    ...
    if len(dr_true) > 0:
        plot_ordinal_confusion_matrix(dr_true, dr_pred, class_names=dr_names,
            output_path=os.path.join(output_dir, "dr_ordinal_confusion_matrix.png"),
            num_classes=num_dr_classes, title=f"DR Ordinal Confusion Matrix - {stage_label}")

    # DR: comprehensive dashboard (2x3)
    if len(dr_true) > 0:
        plot_dr_dashboard(dr_metrics, output_dir=output_dir, num_dr_classes=num_dr_classes,
            dr_true=dr_true, dr_pred=dr_pred, dr_proba=dr_proba)

    # Joint QWK bar chart
    plot_joint_qwk_comparison(dme_qwk_raw=metrics["qwk"], dr_qwk_raw=dr_qwk_raw,
        dr_qwk_calib=dr_qwk_calib, stage_label=stage_label,
        output_path=os.path.join(output_dir, "joint_qwk_comparison.png"))

    # DME: ordinal error histogram
    plot_ordinal_error_histogram(dme_true, dme_pred,
    ...
    if len(dr_true) > 0:
        plot_ordinal_error_histogram(dr_true, dr_pred,
    ...
    plot_per_class_prediction_distribution(dme_true, dme_pred, class_names=dme_names,
    ...
    if len(dr_true) > 0:
        plot_per_class_prediction_distribution(dr_true, dr_pred, class_names=dr_names,
            title=f"DR Per-Class Prediction Distribution - {stage_label}",
            output_path=os.path.join(output_dir, "dr_per_class_pred_dist.png"))

    # DME: multi-class ROC
    plot_multiclass_roc(dme_true, dme_proba, class_names=dme_names,
    ...
    if len(dr_true) > 0 and dr_proba is not None and dr_proba.shape[1] == num_dr_classes:
        plot_multiclass_roc(dr_true, dr_proba, class_names=dr_names,
    ...
    plot_reliability_diagram(dme_true, dme_proba, class_names=dme_names,
    ...
    if len(dr_true) > 0 and dr_proba is not None and dr_proba.shape[1] == num_dr_classes:
        plot_reliability_diagram(dr_true, dr_proba, class_names=dr_names,
    ...
    plot_per_class_f1_bar(dme_true, dme_pred, class_names=dme_names,
    ...
    if len(dr_true) > 0:
        plot_per_class_f1_bar(dr_true, dr_pred, class_names=dr_names,
...
def evaluate_on_best_joint_model(
    ...
    return evaluate_comprehensive(
        ...
        stage_label=stage_label, num_dme_classes=num_dme_classes,
        num_dr_classes=num_dr_classes,
...
def main():
    ...
    if args.model:
        metrics = evaluate_on_best_joint_model(
            checkpoint_path=args.model, dataset=val_ds,
            output_dir=args.output_dir, stage_label=args.stage,
        ...
        metrics = evaluate_comprehensive(model, val_ds, output_dir=args.output_dir,
            stage_label=args.stage, calibrate_dr_thresholds=args.calibrate_dr,
            calibrate_dme_thresholds=args.calibrate_dme)
